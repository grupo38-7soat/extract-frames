...
import os
from constants.constant_env import OUTPUT_FRAMES_DIR_NAME, OUTPUT_ZIP_DIR_NAME, AWS_REGION, DYNAMO_TABLE_NAME, \
    DYNAMO_STATUS_START_PROCESS, \
    DYNAMO_STATUS_DOWNLOAD_VIDEO_IN_PROGRESS, DYNAMO_STATUS_PROCESSING_CUT_FRAMES, DYNAMO_STATUS_ZIP_FILE_IN_PROGRESS, \
    DYNAMO_STATUS_UPLOAD_RESULT_IN_PROGRESS, DYNAMO_STATUS_ERROR, DYNAMO_STATUS_END_PROCESS, OUTPUT_DOWNLOAD_DIR_NAME, \
    RESULT_FOLDER_NAME
...
class SagaControl(object):
    ...
    def run(self):
        try:
            ...
            video_path = f'{current_directory}/{OUTPUT_DOWNLOAD_DIR_NAME}/{self.video_name}.mp4'
            ...
            S3Helper().download_s3_stream(self.video_name, video_path)
            ...
            ExtractFrames(end_time=600, frame_skip=1).run(video_path)
            ...
            S3Helper().upload_file_to_s3(f'{OUTPUT_ZIP_DIR_NAME}/{self.identification}.zip', f'{RESULT_FOLDER_NAME}/{self.user}/{self.identification}.zip')
